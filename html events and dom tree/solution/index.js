...
function createNewNode(event, id) {
  ...
  newParagraph.id = id;
  document.getElementById("injection").appendChild(newParagraph);
  const localObj = new LocalState(newParagraph, 0);
  newParagraph.innerText = `Current State for component with ${id} component is: ${localObj.getCounter()}`;
  mapping[id] = localObj;
}
